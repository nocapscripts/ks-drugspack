...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local dist = #(GetEntityCoords(PlayerPedId()) - vector3(813.34997, -2398.593, 23.805902))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(813.34997, -2398.593, 23.805902, "[E] SISENE KOKAIINI LABI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:EnterCWarehouse')
				--EnterMethlab()
            end
        end
		
		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(1088.7259, -3187.483, -38.99348))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(1088.7259, -3187.483, -38.99348, "[E] VÄLJU KOKAIINI LABIST")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:ExitCWarehouse')
				--EnterMethlab()
            end
        end

		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(1087.2388, -3195.017, -38.99348))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(1087.2388, -3195.017, -38.99348, "[E] PROTSESSI KOKAIINI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:ProcessCocaFarm')
				--EnterMethlab()
            end
        end

		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(1092.89, -3195.78, -38.99))
        if dist < 3 then
  
            QBCore.Functions.DrawText3D(1092.89, -3195.78, -38.99, "[E] PUHASTA KOKAIINI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:ProcessCocaPowder')
				--EnterMethlab()
            end
        end

		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(1100.51, -3199.46, -38.93))
        if dist < 3 then
  
            QBCore.Functions.DrawText3D(1100.51, -3199.46, -38.93, "[E] PAKENDA KOKAIINI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:ProcessBricks')
				--EnterMethlab()
            end
        end
