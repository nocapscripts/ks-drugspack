...
local function ExitMethlab()
    ...
    local keypad = {coords = {x = 996.8543, y = -3200.663, z = -36.39373, h = 95, r = 1.0}}
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local dist = #(GetEntityCoords(PlayerPedId()) - vector3(-1187.084, -446.0578, 43.912372))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(-1187.084, -446.0578, 43.912372, "[E] SISENE METH LABI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:EnterLab')
				--EnterMethlab()
            end
        end
		
		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(996.8543, -3200.663, -36.39373))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(996.8543, -3200.663, -36.39373, "[E] VÄLJU METH LABI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:ExitLab')
				--EnterMethlab()
            end
        end
