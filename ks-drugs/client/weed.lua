...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        local dist = #(GetEntityCoords(PlayerPedId()) - vector3(102.42765, 176.02734, 104.71599))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(102.42765, 176.02734, 104.71599, "[E] SISENE WEED LABI")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:EnterWWarehouse')
				--EnterMethlab()
            end
        end
		
		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(1066.3045, -3183.438, -39.16359))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(1066.3045, -3183.438, -39.16359, "[E] VÄLJU WEED LABIST")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:ExitWWarehouse')
				--EnterMethlab()
            end
        end

		local dist = #(GetEntityCoords(PlayerPedId()) - vector3(1037.6241, -3205.383, -38.17001))
        if dist < 1.5 then
  
            QBCore.Functions.DrawText3D(1037.6241, -3205.383, -38.17001, "[E] PROTSESSI KANEPIT")
            if IsControlJustPressed(1, 38) then
                TriggerEvent('ps-drugprocessing:processWeed')
				--EnterMethlab()
            end
        end
