...
local function PoliceAlert()
	...
	TriggerServerEvent('police:server:policeAlert', 'Narkootikumite müük') -- (DEFAULT QBCore)
	if Config.Debug then print('Teade') end
end
...
local function ShowSellMenu(ped, item, amt, price)
	...
	local SaleMenu = {
		...
		},
		{
			header = "Nõustu",
			params = {
				event = 'ks-drugsell:salesinitiate',
				args = {
					type = 'buy',
					item = item,
					price = price,
	...
	SetTimeout(Config.SellTimeout*1000, function()
		if InitiateSellProgress then
			TriggerEvent("ks-drugsell:notify", "You wasted time so the person left")
...
local function InteractPed(ped)
	...
	if percent < Config.ChanceSell then
		...
		TriggerEvent('ks-drugsell:notify', 'Soovija helistab politseile!!')
...
local function InitiateSales(entity)
	QBCore.Functions.TriggerCallback('ks-drugsell:server:GetCops', function(result)
		if result < Config.MinimumCops then
			TriggerEvent('ks-drugsell:notify', 'Ei ole huvitatud!')
			if Config.Debug then print('Not Enough Cops') end
		else
			local netId = NetworkGetNetworkIdFromEntity(entity)
			local isSoldtoPed = HasSoldPed(netId)
			if isSoldtoPed then TriggerEvent('ks-drugsell:notify', 'Sa juba pakkusid talle') return false end
			AddSoldPed(netId)
			InteractPed(entity)
			if Config.Debug then print('Tegevus') end
		end
	end)
end
...
RegisterNetEvent('ks-drugsell:notify', function(msg)
...
RegisterNetEvent('ks-drugsell:salesinitiate', function(cad)
	if cad.type == 'close' then
		InitiateSellProgress = false
		TriggerEvent("ks-drugsell:notify", "Sa keeldusid")
		TriggerEvent("qb-menu:client:closeMenu")
		SetPedAsNoLongerNeeded(cad.tped)
	else
		InitiateSellProgress = false
		PlayGiveAnim(cad.tped)
		TriggerServerEvent("ks-drugsell:initiatedrug", cad)
		SetPedAsNoLongerNeeded(cad.tped)
	end
end)
...
RegisterNetEvent('ks-drugsell:toggleselling', function()
	AllowedTarget = not AllowedTarget
	if AllowedTarget then
		CreateTarget()
		TriggerEvent("ks-drugsell:notify", "Enabled Selling")
	else
		RemoveTarget()
		TriggerEvent("ks-drugsell:notify", "Disabled Selling")
	end
end)
...
exports[Config.RadialMenu]:AddOption({
	...
	icon = 'cannabis',
	type = 'client',
	event = 'ks-drugsell:toggleselling',
	shouldClose = true
})
end
