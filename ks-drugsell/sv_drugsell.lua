...
QBCore.Functions.CreateCallback('ks-drugsell:server:GetCops', function(source, cb)
	local amount = 0
...
RegisterNetEvent('ks-drugsell:initiatedrug', function(cad)
	local src = source
	...
	if Player then
		...
		if Config.Inventory == 'ox' then
			...
			if item and item >= cad.amt then
				...
				TriggerClientEvent('ks-drugsell:notify', src, 'You recieved $' .. price)
				if Config.Debug then print('You got ' .. cad.amt .. ' ' .. cad.item .. ' for $' .. price) end
			else
				TriggerClientEvent('ks-drugsell:notify', src, 'You could not sell your ' .. cad.item .. '!')
			end
		else
			if Player.Functions.GetItemByName(tostring(cad.item)) then
				...
				TriggerClientEvent('ks-drugsell:notify', src, 'You do not have any ' .. cad.item .. ' to sell!')
			end
		end
	end
end)
...
if Config.ShouldToggleSelling then
	QBCore.Commands.Add("cornersell", "Toggle corner selling (Zone Only)", {}, false, function(source, _)
		TriggerClientEvent("ks-drugsell:toggleselling", source)
	end)
end
